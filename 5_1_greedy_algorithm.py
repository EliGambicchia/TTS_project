...
# This script implements a greedy algorithm that first selects sentences with the highest coverage of phonemes following an ad-hoc wish list
...
def one_recipe_per_line_file():
    ...
    file = open("one_recipe_per_line.txt", "w")
...
def frequency_diphones(ordered_counts_of_diphones):

    # dictionary with all recipes and their diphone sequence as value
    art_dict = 4_exploring_ArcticA.from_arctic_to_diphones()
...
def getting_plot_recipes_before(ordered_counts_of_diphones):
...
def greedy_algorithm(ordered_counts_of_diphones, recipe_dict):
    ...
    return diphones_contained_in_chosen_recipes, new_diphones, list_of_chosen_recipes



def sanity_check_diphones(diphones_contained_in_chosen_recipes):
...
def getting_plot_after_selection(diphones_contained_in_chosen_recipes):
...
def getting_plot_for_diphones(new_diphones):
...
def main():
    ...
    frequency_diphones(ordered_counts_of_diphones)

    getting_plot_recipes_before(ordered_counts_of_diphones)

    diphones_contained_in_chosen_recipes, new_diphones, list_of_chosen_recipes = greedy_algorithm(ordered_counts_of_diphones, recipe_dict)

    sanity_check_diphones(diphones_contained_in_chosen_recipes)

    getting_plot_after_selection(diphones_contained_in_chosen_recipes)

    getting_plot_for_diphones(new_diphones)

    file = open("chosen_recipes.txt", "w") 
